...
class Level5Screen extends flutter.StatefulWidget {
  const Level5Screen({super.key});

  @override
  flutter.State<Level5Screen> createState() => _Level5ScreenState();
}

class _Level5ScreenState extends flutter.State<Level5Screen> {
  final Level5Game _game = Level5Game();
        ...
        children: [
          if (!_showQuestion)
            GameWidget(
              game: _game,
              overlayBuilderMap: {
                'Level5CompleteOverlay': (flutter.BuildContext context, Level5Game game) {
                  return lvCompleted.LevelCompleteOverlay(
                    ...
                    onBackPressed: () {
                      game.overlays.remove('Level5CompleteOverlay');
                ...
                'PointsDisplay': (flutter.BuildContext context, Level5Game game) {
                ...
                'AlertMessage': (flutter.BuildContext context, Level5Game game) {
                  return flutter.Positioned(
                    top: 50,
                    left: 0,
                    right: 0,
...
class Level5Game extends FlameGame with DragCallbacks, HasCollisionDetection implements PointCollector {
  ...
  void collectPoint() {
    ...
    if (collectedPoints >= totalPoints && !_levelCompleted) {
      ...
      overlays.add('Level5CompleteOverlay');
  ...
  Future<void> _loadMap() async {
    map = await TiledComponent.load('level5.tmx', Vector2.all(tileSize));
    world.add(map);
